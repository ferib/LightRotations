...
local function buildElements(table, parent)
  ...
  for _, element in ipairs(table.template) do
    ...
    elseif element.type == 'spinner' then
      ...
      tmp_spin:SetNumber(
        dank.settings.fetch(table.key .. '_' .. element.key, element.default)
      )
    ...
    elseif element.type == 'checkspin' then
      ...
      tmp_spin:SetNumber(
        dank.settings.fetch(table.key .. '_' .. element.key .. '.spin', element.default_spin or 0)
      )
      if tmp_spin.SetStylesheet then tmp_spin:SetStylesheet(spinnerStyleSheet) end
      tmp_spin:ApplySettings()
      ...
      tmp_check:SetChecked(dank.settings.fetch(table.key .. '_' .. element.key .. '.check', element.default_check or false))
...
function builder.buildGUI(template)
  ...
  if template.profiles == true and template.key_orig then
    ...
    createButton:SetEventListener('OnClick', function()
      ...
      profileButton:SetEventListener('OnClick', function()
        ...
        local profiles = dank.settings.fetch(template.key_orig .. '_' .. 'profiles', {{key='default',text='Default'}})
  ...
  if not template.color then template.color = dank.color end
...
dank.on_ready(function()
  local engine = {
    ...
    template = {
      ...
      { key = 'enablelogfile', type = 'checkbox', text = 'Enable log to file', default = false },
      { key = 'logfilename', type = 'input', text = 'Logfile Name', width = 170.0 },
      { key = 'secdebug', type = 'checkbox', text = 'Debug Authentication', default = false },
