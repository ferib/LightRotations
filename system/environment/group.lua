...
local function group_count(func)
  local count = 0
  for unit in dank.environment.iterator() do
    if func(unit) then 
      count = count + 1
    end
  end
...
local function check_removable(removable_type)
  return group_match(function (unit)
    local debuff, count, duration, expires, caster, found_debuff = UnitReverseDebuff(unit.unitID, dank.data.removables[removable_type])
...
function dank.environment.conditions.group()
  return setmetatable({ }, {
