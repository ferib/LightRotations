...
local UnitBuff = light.environment.unit_buff
...
function light.environment.conditions.buff(unit)
    return setmetatable({
        unitID = unit.unitID
    }, {
        __index = function(self, func)
            local result = buff[func](self)
            light.console.debug(4, 'buff', 'green', self.unitID .. '.buff(' .. tostring(self.spell) .. ').' .. func ..
                ' = ' .. light.format(result))
            return result
        end,
        ...
        __unm = function(t)
            ...
            light.console.debug(4, 'buff', 'green',
                t.unitID .. '.buff(' .. tostring(t.spell) .. ').exists = ' .. light.format(result))
            return result
        end
    })
end
