...
local function find_gcd_spell()
  ...
  C_Timer.After(0.5, function()
    if not gcd_spell then
      ...
      dank.console.debug(4, 'engine', 'engine', string.format('GCD candidate found, using %s (%s)', gcd_spell, gcd_spell_name))
    end
  end)
end
...
function dank.rotation.tick(ticker)
  ...
  if dank.rotation.active_rotation then
    ...
    if dank.savedHealTarget ~= nil and not iscasting then
      ...
      dank.console.debug(1, 'engine', 'engine', string.format('finished casting spell on %s. Health %d start cooldown of %1.1f seconds', UnitName(dank.savedHealTarget), UnitHealth(dank.savedHealTarget), hcd))
    ...
    for unit, cdtime in pairs(dank.healthCooldown) do
      if GetTime() > cdtime then
        if UnitName(unit) ~= nil and UnitHealth(unit) ~= nil then
          dank.console.debug(1, 'engine', 'engine', string.format('cooldown finished for unit %s health %d', UnitName(unit), UnitHealth(unit)))
        end
    ...
    if UnitAffectingCombat('player') then
      ...
      if GetTime() - last_loading > loading_wait then
        dank.interface.status_override(
          dank.interface.loading_messages[math.random(#dank.interface.loading_messages)],
          10
        )
